...
<section class="faq-section" id="faq">
  <div class="faq-container">
    <div class="faq-header">
      <h2 {!! content_typography_html('faq.home_preview.heading') !!}>{{ content_text('faq.home_preview.heading', 'Frequently Asked Questions') }}</h2>
      <a href="{{ route('faq.show') }}" class="faq-read-more">{{ content('faq.home_preview.read_more_label', 'Read more') }}</a>
    </div>
    <div class="faq-layout">
      <p {!! content_typography_html('faq.home_preview.subtitle', 'faq-subtitle') !!}>{{ content_text('faq.home_preview.subtitle', 'How it works') }}</p>
        ...
        @foreach ($faqItems as $idx => $item)
          ...
          <div class="faq-item @if($isOpen) active @endif">
            <button class="faq-question" aria-expanded="{{ $isOpen ? 'true' : 'false' }}">
              <span {!! content_typography_html($qKey) !!}>{{ content_text($qKey, $item['question'] ?? '') }}</span>
            ...
            <div class="faq-answer">
              <p {!! content_typography_html($aKey) !!}>{{ content_text($aKey, $item['answer'] ?? '') }}</p>
            </div>
          </div>
        @endforeach
      </div>
